#!/usr/bin/env bash
# forgejo-restore-data.sh [backup-file-name]
#
# Replaces Forgejo's application data with one of the archives the backups
# service wrote.
#
#   ./forgejo-restore-data.sh               list and ask
#   ./forgejo-restore-data.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than forgejo.
set -Eeuo pipefail
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Forgejo." >&2
  exit 1
fi
