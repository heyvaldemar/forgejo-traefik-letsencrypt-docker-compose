#!/bin/bash
# End-to-end tests for the forgejo-traefik-letsencrypt-docker-compose
# backup + restore flow.
#
# This application keeps everything it knows in one directory: the git
# repositories themselves, a SQLite file holding users, issues and tokens, and
# app.ini beside them. There is no database server to dump, so the archive IS
# the instance, and the test that matters is whether a file written before a
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-forgejo}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-forgejo-traefik-letsencrypt-docker-compose.yml}"
...
: "${DATA_BACKUPS_PATH:=/srv/forgejo-data/backups}"
: "${DATA_BACKUP_NAME:=forgejo-data-backup}"
: "${DATA_PATH:=/data}"
: "${BACKUP_INTERVAL:=24h}"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq forgejo | head -n 1)"
[[ -n "$BACKUPS_CONTAINER" ]] || { echo "error: backups container not found" >&2; exit 1; }
[[ -n "$APP_CONTAINER" ]] || { echo "error: forgejo container not found" >&2; exit 1; }
...
test_env_required() {
  grep -qE '^FORGEJO_HOSTNAME=.' .env || { echo "  FORGEJO_HOSTNAME is not set in .env" >&2; return 1; }
...
test_backup_carries_the_data_directory() {
  local f; f=$(list_backups | tail -1)
  bk "tar -tzf '$f' | grep -q '^data/'" || { echo "  $f does not carry the data directory" >&2; return 1; }
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./forgejo-restore-data.sh "$(basename "$f")" > /dev/null \
    || { echo "  ./forgejo-restore-data.sh failed" >&2; return 1; }
